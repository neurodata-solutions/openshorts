...
def root_dir():
    """
    Overridden from upstream MPT.

    Upstream ``root_dir()`` points at the MPT repo root (three dirs up from
    ``app/utils/utils.py``) and everything else in this module
    (``storage_dir``, ``task_dir``, the material search cache, ...) is
    derived from it. topic_video/ is not its own repo -- it's a package
    inside OpenShorts -- so instead of writing MPT-style scratch state
    (``storage/tasks/``, ``storage/cache_material_search/``, ...) into the
    OpenShorts repo root, all of it is confined under OpenShorts' own
    ``output/`` directory (``output/_mpt_cache/...``), which OpenShorts
    already treats as disposable, writable job-output space (see
    ``OUTPUT_DIR`` in openshorts/app.py) rather than repo source.
    """
    openshorts_root = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    return os.path.join(openshorts_root, "output", "_mpt_cache")
...
def font_dir(sub_dir: str = ""):
    ...
    Upstream resolves to ``resource_dir("fonts")`` (MPT ships CJK fonts like
    STHeitiMedium.ttc there, matching MPT's own ``VideoParams.font_name``
    default). OpenShorts does not vendor those MPT font files; it ships its
    own font set in ``openshorts/fonts/`` (already used by the clip
    generator's hook/subtitle overlays), which includes ``NotoSerif-Bold.ttf``.
    topic_video/pipeline.py sets ``VideoParams.font_name`` to
    ``"NotoSerif-Bold.ttf"`` so ``os.path.join(utils.font_dir(), font_name)``
    in video.py resolves to a font file that actually exists in this repo.
    """
    openshorts_root = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    d = os.path.join(openshorts_root, "fonts")
    if sub_dir:
        d = os.path.join(d, sub_dir)
    return d
